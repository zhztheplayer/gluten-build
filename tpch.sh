...
# Run gdbfrontend server.
RUN_GDBFRONTEND_SERVER=${RUN_GDBFRONTEND_SERVER:-$DEFAULT_RUN_GDBFRONTEND_SERVER}

# Run JVM jdwp server.
RUN_JDWP_SERVER=${RUN_JDWP_SERVER:-$DEFAULT_RUN_JDWP_SERVER}

# Validate debug options
if [ "$RUN_GDB_SERVER" == "ON" ] && [ "$RUN_GDBFRONTEND_SERVER" == "ON" ]
then
  echo "RUN_GDB_SERVER and RUN_GDBFRONTEND_SERVER cannot be set to ON at the same time"
  exit 1
fi
...
elif [ "$RUN_GDBFRONTEND_SERVER" == "ON" ]
then
  DOCKER_SELECTED_TARGET_IMAGE_TPCH=${DOCKER_TARGET_IMAGE_TPCH_GDBFRONTEND_SERVER:-$DEFAULT_DOCKER_TARGET_IMAGE_TPCH_GDBFRONTEND_SERVER}
  DOCKER_BUILD_TARGET_NAME=gluten-tpch-gdbfrontend-server
else
...
# gdbfrontend server bind port
GDBFRONTEND_SERVER_PORT=${GDBFRONTEND_SERVER_PORT:-$DEFAULT_GDBFRONTEND_SERVER_PORT}
...
then
  BASH_ARGS="$BASH_ARGS gdbfrontend --dontopenuionstartup --listen=0.0.0.0 --port=$GDBFRONTEND_SERVER_PORT --gdb-args=\"--args java $JAVA_ARGS\""
else
